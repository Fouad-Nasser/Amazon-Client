                ...
                <div id="nav-bar ">
                    <div id="nav-belt">
                                ...
                                <div id="nav-search-bar-form">
                                    <div className="nav-fill">
                                        <div id='DivHeaderSearchBar'>
                                            <input id="header__searchInput" type="text" placeholder={t("searchAmazon")} onClick={overlaySearchBar} onChange={(e) => { handleChange(e.target.value) }} />
                                        </div>
                                        <div id="SearchResultList" style={{ zIndex: '60' }}>
                                            {
                                                searchDataResult.map((result) => {
                                                    return <div className='d-flex' onClick={() => { console.log(result.name); }}>
                                                        <NavLink to={`/product/${result._id}}`} className='text-decoration-none searchListResult'>
                        ...
                        <div className="nav-right">
                            <div >
                                <Dropdown>
                                    <Dropdown.Toggle variant="rimary" id="dropdown-basic" className='text-white' style={{ margin: '0px', padding: '6px' }}>
                                        <span className={`flag-icon flag-icon-${currentLanguage.country_code} mx-2`}></span>
                                        {currentLanguageCode}
                                    </Dropdown.Toggle>
                                    <Dropdown.Menu>
                                        {
                                            languages.filter(i => i.code !== currentLanguage.code)
                                                .map(i =>
                                                    <Dropdown.Item>
                                                        ...
                                                        <li className="dropdown-item" onClick={() => { i18n.changeLanguage(i.code); window.location.reload() }} >
                                                            <span className={`flag-icon flag-icon-${i.country_code} mx-2`} ></span>
                                                            {i.name}
                                                        </li>
                                                    </Dropdown.Item>
                            ...
                            <NavLink className='hideDatainResp text-white mt-1 text-decoration-none' to="/Signin">
                                <div className="header__option">
                                    <span className="header__optionLineOne">{t('HelloGuest')}</span>
                                    <NavDropdown title={t('SingIn')} id="navbarScrollingDropdown">
                                        <div id='navbarScrollingDropdownHeader'>
                                            <div className='row'>
                                                <Row>
                                                    <Col></Col>
                                                    <Col><Link to="/SignIn"><a href="#" class="myButton">{t('SingIn')}</a></Link></Col>
                                                    <Col></Col>
                                                </Row>
                                                <div className='row' id='headerNewCustomer'>
                                                    <span id='headerNewCustomerSpan'>{t('NewCutomer')} <Link to="/CreateAccount">{t('StartHere')}</Link></span>
                                                </div>
                                            </div>
                                            <hr id='hrSignIn' />
                                            <Row>
                                                <Col id='ColNav' className='ColNav1'>
                                                    <h4> {t('YourLists')}</h4>
                                                    <div><a href='#'>{t('CreateList')}</a></div>
                                                </Col>
                                                <Col xs={2}><div class="vl"></div></Col>
                                                <Col id='ColNav' >
                                                    <h4> {t('YourAccount')}</h4>
                                                    <Link to="/Account"><a href='#'>{t('YourAccount')}</a></Link>
                                                    <div><a href='#'>{t('Orders')}</a></div>
                                                    <div><a href='#'>{t('Recomindations')}</a></div>
                                                    <div><a href='#'>{t('Address')}</a></div>
                                                    <div><a href='#'>{t('Watchlist')}</a></div>
                                                    <div><a href='#'>{t('Subscribe&saveItems')}</a></div>
                                                    <div><a href='#'>{t('PrimeMemberShips')}</a></div>
                                                    <div><a href='#'>{t('SellerAccount')}</a></div>
                                                </Col>
                                            </Row>
                                        </div>
                                    </NavDropdown>
                            ...
                            <NavLink className='hideRes280data text-white mt-1 text-decoration-none' to="/Order">
                                <div className="header__option">
                            ...
                            <NavLink className='text-white mt-1 text-decoration-none nav-a nav-a-2 nav-progressive-attribute' aria-label="0 items in cart" id="nav-cart" to="/Cart">
                                <div id="header__optionBasket">
                                    <div id="nav-cart-count-container">
                                        <span className="nav-cart-icon nav-sprite"><span id='stext-warning'>{cartItems.length}</span></span>
                                    </div>
                                    <div id="nav-cart-text-container" className="nav-progressive-attribute" style={{ marginTop: '-2.5rem;' }}>
                    ...
                    <div className="p-1 nav_bar text-white">
                        <div className="left d-flex">
                            ...
                            <NavLink className='hideDatainResp text-white mt-1 text-decoration-none' to="/">
                                <span className='p-1 px-2 cart-data'>{t('CustomerService')}</span>
                            </NavLink>
